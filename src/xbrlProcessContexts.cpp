...
RcppExport SEXP xbrlProcessContexts(SEXP epaDoc) {
  ...
  xmlXPathObjectPtr context_res =
      xmlXPathEvalExpression((xmlChar *)"//*[local-name()='context']", context);
  ...
  for (int i = 0; i < context_nodeset_ln; i++) {
    ...
    scheme[i] = identifier[i] = startDate[i] = endDate[i] = dimension1[i] =
        value1[i] = dimension2[i] = value2[i] = dimension3[i] = value3[i] =
            dimension4[i] = value4[i] = NA_STRING;
      ...
      if (!xmlStrcmp(child_node->name, (xmlChar *)"entity")) {
        ...
        while (gchild_node) {
          if (!xmlStrcmp(gchild_node->name, (xmlChar *)"identifier")) {
            ...
            if ((tmp_str = xmlNodeListGetString(
                     doc, gchild_node->xmlChildrenNode, 1))) {
            ...
            while (ggchild_node) {
              if (!xmlStrcmp(ggchild_node->name, (xmlChar *)"explicitMember")) {
                if ((tmp_str =
                         xmlGetProp(ggchild_node, (xmlChar *)"dimension"))) {
                  if (dimn == 1)
                ...
                if ((tmp_str = xmlNodeListGetString(
                         doc, ggchild_node->xmlChildrenNode, 1))) {
                  if (dimn == 1)
  ...
  return DataFrame::create(
      Named("contextId") = contextId, Named("scheme") = scheme,
      Named("identifier") = identifier, Named("startDate") = startDate,
      Named("endDate") = endDate, Named("dimension1") = dimension1,
      Named("value1") = value1, Named("dimension2") = dimension2,
      Named("value2") = value2, Named("dimension3") = dimension3,
      Named("value3") = value3, Named("dimension4") = dimension4,
      Named("value4") = value4);
}
