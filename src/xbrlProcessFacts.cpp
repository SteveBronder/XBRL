...
RcppExport SEXP xbrlProcessFacts(SEXP epaDoc) {
  ...
  xmlXPathObjectPtr fact_res = xmlXPathEvalExpression(
      (xmlChar *)"//*[@*[local-name()='contextRef']]", context);
  ...
  for (int i = 0; i < fact_nodeset_ln; i++) {
    ...
    if (fact_node->ns->prefix)
      elementId[i] = (char *)((string)(char *)fact_node->ns->prefix + "_" +
                              (string)(char *)fact_node->name)
                         .data();
    else
  ...
  return DataFrame::create(
      Named("elementId") = elementId, Named("contextId") = contextId,
      Named("unitId") = unitId, Named("fact") = fact,
      Named("decimals") = decimals, Named("scale") = scale,
      Named("sign") = sign, Named("factId") = factId, Named("ns") = ns);
}
