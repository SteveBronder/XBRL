...
RcppExport SEXP xbrlProcessUnits(SEXP epaDoc) {
  ...
  xmlXPathObjectPtr unit_res =
      xmlXPathEvalExpression((xmlChar *)"//*[local-name()='unit']", context);
    ...
    while (child_node) {
      if (!xmlStrcmp(child_node->name, (xmlChar *)"measure")) {
        ...
        while (gchild_node) {
          ...
          } else if (!xmlStrcmp(gchild_node->name,
                                (xmlChar *)"unitDenominator")) {
            xmlNodePtr ggchild_node = gchild_node->xmlChildrenNode;
            while (ggchild_node) {
              if (!xmlStrcmp(ggchild_node->name, (xmlChar *)"measure")) {
                if ((tmp_str = xmlNodeListGetString(
  ...
  return DataFrame::create(Named("unitId") = unitId, Named("measure") = measure,
