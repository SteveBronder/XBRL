...
RcppExport SEXP xbrlProcessFootnotes(SEXP epaDoc) {
  ...
  xmlXPathObjectPtr footnoteArc_res = xmlXPathEvalExpression(
      (xmlChar *)"//*[local-name()='footnoteArc']", context);
  ...
  xmlXPathObjectPtr footnote_res = xmlXPathEvalExpression(
      (xmlChar *)"//*[local-name()='footnote']", context);
  ...
  for (int i = 0; i < footnoteArc_nodeset_ln; i++) {
    ...
    for (int j = 0; j < loc_nodeset->nodeNr; j++) {
      ...
      for (int k = 0; k < footnote_nodeset->nodeNr; k++) {
        ...
        if ((tmp_str = xmlNodeListGetString(doc, footnote_node->xmlChildrenNode,
                                            1))) {
  ...
  return DataFrame::create(Named("factId") = factId, Named("lang") = lang,
